...
export default function TarjetaIndicador({ ficha, onAbrir }) {
  ...
  return (
    <button
      ...
      className="text-left bg-white rounded-xl border border-borde p-5 hover:shadow-md hover:border-acento/40 transition w-full flex flex-col"
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-acento-fuerte leading-snug">{ficha.nombre}</p>
      ...
      <div className="flex items-end justify-between gap-3 mt-auto">
        <div>
          <p className="text-3xl font-bold text-acento-fuerte leading-none">
            {formatValor(ficha.valor, ficha.unidad)}
          </p>
          <div className="flex items-center gap-2 mt-1.5 text-[11px]">
            {ficha.meta !== null && (
              <span className="text-texto-3">Meta {formatValor(ficha.meta, ficha.unidad)}</span>
            )}
            {variacion && (
      ...
      {ficha.valor === null && !ficha.es_automatico && (
        <p className="text-[11px] text-alerta bg-alerta-bg border border-ambar/30 rounded-lg px-2 py-1 mt-3">

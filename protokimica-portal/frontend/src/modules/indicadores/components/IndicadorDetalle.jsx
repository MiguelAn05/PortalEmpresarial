...
export default function IndicadorDetalle({ indicadorId, anio, mes, editable, onEditar, onCerrar }) {
  ...
  return (
    <div className="fixed inset-0 bg-acento-fuerte/40 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={onCerrar}>
      <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[92vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
        {isLoading || !ficha ? (
          <div className="p-16 text-center text-sm text-texto-3">Cargando indicador...</div>
        ) : (
          <>
            <div className="bg-gradient-to-r from-acento-fuerte to-acento rounded-t-2xl p-6 text-white sticky top-0 z-10">
              <button onClick={onCerrar} aria-label="Cerrar" className="absolute top-4 right-4 text-white/70 hover:text-white"><IconoCerrar tam={18} /></button>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <p className="text-4xl font-bold text-acento-fuerte leading-none">
                ...
                <div className="flex gap-2">
                  ...
                  {editable && ficha.es_automatico && (
                    <button
                      ...
                      className="border border-borde hover:bg-superficie-2 text-acento-fuerte text-sm font-semibold px-4 py-2.5 rounded-xl transition disabled:opacity-40"
                    >
              ...
              {/* Comparaciones */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-superficie-2 border border-borde rounded-xl overflow-hidden">
              ...
              {/* Tendencia */}
              <div>
                <h3 className="text-sm font-bold text-acento-fuerte mb-3">Tendencia {anio}</h3>
                <GraficaTendencia serie={ficha.serie} unidad={ficha.unidad}
                  meta={ficha.meta} mesActual={mes} />
              </div>

              {/* Ficha técnica */}
              <div className="bg-superficie-2 rounded-xl p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {ficha.evidencia && (
                  <div>
                    ...
                    <a href={ficha.evidencia} target="_blank" rel="noreferrer"
                      className="text-sm text-acento font-semibold underline">
                      <IconoClip tam={13} /> Ver soporte adjunto
                    </a>
                  </div>
                )}
              </div>
              ...
              <div className="border-t border-borde pt-5">
                <h3 className="text-sm font-bold text-acento-fuerte mb-3">Correcciones al valor</h3>
                {historial.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {historial.map(h => (
                      <div key={h.id} className="border-l-2 border-borde pl-4 py-1">
                        ...
                        <p className="text-sm flex flex-wrap items-baseline gap-x-1.5">
                          <span className="line-through text-texto-3">{formatValor(h.valor_anterior, ficha.unidad)}</span>
                          <span className="text-texto-3">→</span>
                          <span className="font-semibold text-texto">{formatValor(h.valor_nuevo, ficha.unidad)}</span>
                        </p>
                        {h.motivo && <p className="text-xs text-texto-2 mt-0.5">{h.motivo}</p>}
                      </div>
                    ))}
                  </div>
                )}
...
function Comparacion({ titulo, valor, variacion, unidad, direccion }) {
  ...
  return (
    <div className="bg-white px-4 py-3">
      <p className="text-[11px] font-semibold text-texto-3 uppercase tracking-wide">{titulo}</p>
      <p className="text-base font-bold text-acento-fuerte mt-1">{formatValor(valor, unidad)}</p>
      {texto && (
...
function Acumulado({ titulo, acc, unidad, modo }) {
  return (
    <div className="bg-white px-4 py-3">
      <p className="text-[11px] font-semibold text-texto-3 uppercase tracking-wide">{titulo}</p>
      <p className="text-base font-bold text-acento-fuerte mt-1">{formatValor(acc.valor, unidad)}</p>
      <p className="text-[11px] text-texto-3">
...
function Campo({ titulo, valor }) {
  return (
    <div>
      ...
      <p className="text-sm text-texto whitespace-pre-line">
        {valor || <span className="text-borde-fuerte italic">Sin definir</span>}
      </p>
    </div>
  )
}

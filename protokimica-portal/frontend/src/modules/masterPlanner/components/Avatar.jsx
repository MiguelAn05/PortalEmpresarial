export default function Avatar({ name, compact }) {
  if (!name) {
    return compact ? (
      <span className="text-[11px] text-[#9BACC8] italic">Sin asignar</span>
    ) : (
      <span className="text-sm text-[#9BACC8] italic">Sin asignar</span>
    )
  }
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-9 h-9 rounded-full bg-[#1A4FA0] text-white flex items-center justify-center font-semibold text-sm">
        {initials}
      </div>

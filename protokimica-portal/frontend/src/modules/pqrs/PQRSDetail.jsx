    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="col-span-1 space-y-5">
          ...
          <div className="bg-white rounded-xl border border-borde p-5">
            <div className="flex items-center justify-between gap-2 mb-4">
              <h3 className="font-semibold text-acento-fuerte text-sm">
                {(pqrs.productos?.length ?? 0) > 1 ? `Productos (${pqrs.productos.length})` : 'Producto y factura'}
              </h3>
              {puedeEditarDatos && (
                <BotonEditar onClick={() => setEditandoDatos(true)} etiqueta="Editar el número de factura" />
              )}
            </div>

            {/* Cada producto con su lote y cantidades. El que escribió el
                cliente a mano se confirma aquí contra el catálogo: después de
                cerrar ya no se puede, y un nombre suelto vuelve inservible el
            ...
            <div className="space-y-3 mt-4 pt-4 border-t border-borde">
              {[

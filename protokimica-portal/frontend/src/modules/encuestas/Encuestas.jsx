  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 py-8 space-y-6">
      <div className="flex flex-wrap justify-between items-start gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#0D2B5E]">Encuestas</h1>
        ...
        {editable && pestana === 'encuestas' && (
          <button
            ...
            className="bg-[#F5A800] hover:bg-[#FFC840] text-[#0D2B5E] font-semibold px-5 py-2.5 rounded-xl shadow-sm transition"
          >
            + Nueva encuesta
          </button>
        )}
      ...
      {pestana === 'respuestas' && (
        isLoading ? (
          <div className="text-center py-16 text-[#9BACC8] text-sm">Cargando respuestas...</div>
        ) : panel && (
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-3">
        <label className="text-xs font-semibold text-[#6B7EA8] uppercase">Encuesta</label>
        <select
          value={origen}
          onChange={(e) => onOrigen(e.target.value)}
          className="rounded-lg border border-[#D6E0F0] px-3 py-2 text-sm bg-white"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-[#D6E0F0] border-t-4 border-t-[#1A4FA0] p-4">
          <div className="text-xs font-semibold text-[#6B7EA8] uppercase tracking-wide">Calificación</div>
          <div className={`text-3xl font-bold mt-1 tabular-nums ${NIVELES[nivel].texto}`}>
            {formatNota(resumen.promedio)}
            <span className="text-base font-semibold text-[#9BACC8]"> / {resumen.escala_max}</span>
          </div>
          <div className="text-[11px] mt-0.5">
            <span className="inline-flex items-center gap-1">
              <span className="w-2 h-2 rounded-full inline-block" style={{ background: NIVELES[nivel].punto }} aria-hidden="true" />
              <span className="text-[#6B7EA8]">{NIVELES[nivel].label}</span>
            </span>
          </div>
        </div>

        <div className="bg-white rounded-xl border border-[#D6E0F0] border-t-4 border-t-[#0D2B5E] p-4">
          <div className="text-xs font-semibold text-[#6B7EA8] uppercase tracking-wide">Respuestas</div>
          <div className="text-3xl font-bold mt-1 text-[#0D2B5E] tabular-nums">{resumen.total}</div>
          <div className="text-[11px] text-[#9BACC8] mt-0.5">{resumen.calificadas} con calificación</div>
        </div>

        <div className="bg-white rounded-xl border border-[#D6E0F0] border-t-4 border-t-[#D93B3B] p-4">
          <div className="text-xs font-semibold text-[#6B7EA8] uppercase tracking-wide">Insatisfechos</div>
          <div className="text-3xl font-bold mt-1 text-[#D93B3B] tabular-nums">{resumen.detractores}</div>
          <div className="text-[11px] text-[#9BACC8] mt-0.5">
            {resumen.detractores_pct !== null ? `${resumen.detractores_pct}% calificó 1 o 2` : 'sin calificaciones'}
          </div>
        </div>

        <div className="bg-white rounded-xl border border-[#D6E0F0] border-t-4 border-t-[#F5A800] p-4">
          <div className="text-xs font-semibold text-[#6B7EA8] uppercase tracking-wide">Comentarios</div>
          <div className="text-3xl font-bold mt-1 text-[#0D2B5E] tabular-nums">{resumen.con_comentario}</div>
          <div className="text-[11px] text-[#9BACC8] mt-0.5">para leer</div>
        </div>
      </div>
...
function Distribucion({ resumen }) {
  ...
  return (
    <section className="bg-white rounded-xl border border-[#D6E0F0] p-5">
      <h3 className="font-bold text-[#0D2B5E]">Cómo se reparten las notas</h3>
        ...
        {[...Array(ESCALA_MAX)].map((_, i) => {
          ...
          return (
            <div key={nota} className="grid grid-cols-[2.5rem_1fr_2.5rem] gap-2 items-center">
              <span className="text-xs text-[#42557A] text-right tabular-nums">{nota} ★</span>
              <span className="h-4 bg-[#EEF2F8] rounded overflow-hidden">
                <span className="block h-full bg-[#1A4FA0] rounded" style={{ width: `${pct}%` }} />
              </span>
              <span className="text-xs font-bold tabular-nums text-right text-[#1A2B47]">{cuantos}</span>
            </div>
          )
        })}
      </div>
...
function Ranking({ filas }) {
  return (
    <section className="bg-white rounded-xl border border-[#D6E0F0] p-5">
      <h3 className="font-bold text-[#0D2B5E]">Calificación por persona o punto</h3>
      <p className="text-xs text-[#6B7EA8] mb-3">
        ...
        {filas.map(f => {
          ...
          return (
            <li key={f.sujeto} className="flex items-center justify-between gap-3 py-2">
              <span className="text-sm text-[#1A2B47] truncate">{f.sujeto}</span>
              <span className="flex items-center gap-2 shrink-0">
                <span className="text-[11px] text-[#9BACC8] tabular-nums">
                  {f.respuestas} resp.
                </span>
  ...
  if (respuestas.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-dashed border-[#D6E0F0] p-16 text-center">
        <p className="text-[#6B7EA8]">Todavía no hay respuestas en este filtro.</p>
      </div>
    )
  }

  return (
    <section className="bg-white rounded-xl border border-[#D6E0F0] overflow-hidden">
      <div className="px-5 py-4 border-b border-[#D6E0F0]">
        <h3 className="font-bold text-[#0D2B5E]">Respuestas</h3>
        <p className="text-xs text-[#6B7EA8]">De la más reciente a la más antigua.</p>
      </div>
          ...
          return (
            <li key={r.id}>
              <button
                ...
                className="w-full px-5 py-3 flex items-start justify-between gap-4 text-left hover:bg-[#F7F9FC] transition"
              >
                <span className="min-w-0">
                  <span className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[11px] text-[#6B7EA8] bg-[#F7F9FC] border border-[#D6E0F0] rounded-full px-2">
                      {r.origen_nombre}
                    </span>
                    {r.referencia && (
                      <span className="text-[11px] text-[#9BACC8]">{r.referencia}</span>
                    )}
                  </span>
                  {r.comentario && (
                    <span className={`block text-sm text-[#42557A] mt-1 ${abierto ? '' : 'truncate'}`}>
                      «{r.comentario}»
                    </span>
                  )}
                </span>
              ...
              {abierto && (
                <div className="px-5 pb-4 -mt-1">
                  <dl className="bg-[#F7F9FC] border border-[#D6E0F0] rounded-lg p-3 space-y-1.5">
                    {r.items.map((item, i) => (
                      <div key={i} className="grid grid-cols-[1fr_auto] gap-3">
                        <dt className="text-xs text-[#6B7EA8]">{item.pregunta}</dt>
                        <dd className="text-xs font-semibold text-[#1A2B47] text-right">{item.valor}</dd>
                      </div>
                    ))}
                  </dl>
                </div>
      ...
      {hayMas && (
        <p className="px-5 py-3 text-xs text-[#6B7EA8] border-t border-[#D6E0F0]">
...
function ListaPlantillas({ plantillas, editable, onEditar }) {
  ...
  if (plantillas.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-dashed border-[#D6E0F0] p-16 text-center">
        <p className="text-[#6B7EA8] mb-2">Todavía no has creado ninguna encuesta.</p>
        <p className="text-sm text-[#9BACC8]">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      {plantillas.map(p => (
        <div key={p.id} className="bg-white rounded-xl border border-[#D6E0F0] p-5 flex flex-col gap-3">
          <div>
            <div className="flex items-start justify-between gap-2">
              <h3 className="font-bold text-[#0D2B5E]">{p.nombre}</h3>
              {!p.activa && (
                <span className="text-[11px] font-semibold text-[#6B7EA8] bg-[#F7F9FC] border border-[#D6E0F0] rounded-full px-2 py-0.5 shrink-0">
                  Inactiva
                </span>
              )}
            </div>
            {p.descripcion && <p className="text-sm text-[#6B7EA8] mt-1">{p.descripcion}</p>}
          </div>

          <div className="text-xs text-[#6B7EA8] flex gap-3">
            <span>{p.preguntas.length} pregunta{p.preguntas.length === 1 ? '' : 's'}</span>
            <span>·</span>
            <span className="font-semibold text-[#1A2B47] tabular-nums">
              {p.total_respuestas} respuesta{p.total_respuestas === 1 ? '' : 's'}
            </span>
          </div>

          {/* El enlace es lo que se imprime en el QR del punto de venta */}
          <div className="bg-[#F7F9FC] border border-[#D6E0F0] rounded-lg px-3 py-2 flex items-center gap-2">
            <code className="text-[11px] text-[#42557A] truncate flex-1">{urlPublica(p.slug)}</code>
            <button
              onClick={() => copiar(p.slug)}
              className="text-[11px] font-semibold text-[#1A4FA0] hover:underline shrink-0"
            >
          ...
          {editable && (
            <button
              onClick={() => onEditar(p)}
              className="text-sm font-semibold text-[#1A4FA0] hover:underline text-left"
            >
              Editar
            </button>
          )}
